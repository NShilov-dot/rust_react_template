...
/// generates a CSRF state token + PKCE verifier; the application layer is
/// responsible for stashing the verifier under the state key in a cache
...
pub trait GoogleAuthClient: Send + Sync {
    /// Build the Google authorize URL with PKCE + state. Returns secrets
    /// the caller must store before the redirect.
    fn authorize(&self) -> AuthRequest;

    /// Exchange an authorization `code` for an access token, then call
    /// Google's userinfo endpoint. `pkce_verifier` must match what was
    /// generated by `authorize()` for the same state.
    async fn exchange(&self, code: &str, pkce_verifier: &str)
